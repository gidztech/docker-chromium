...
const dockerBuild = async () => {
    ...
    await runCommand('docker-compose', [
        ...
        'build',
        `--build-arg`,
        `CHROMIUM_ADDITIONAL_ARGS="${chromiumAdditionalArgs}"`,
        `--build-arg`,
        `CHROMIUM_DOWNLOAD_HOST=${chromiumDownloadHost}`,
        `--build-arg`,
        `CHROMIUM_REVISION=${chromiumRevision || DEFAULT_CHROMIUM_REVISION}`,
        '--pull',
        serviceToBuild
    ])
        .then(() => {
            console.log(
                `${CONSOLE_PREFIX} Successfully built Docker image`.green
            );
        })
        .catch(error => {
            throw new Error(
...
const dockerDown = async () => {
    ...
    await runCommand('docker-compose', ['-f', `${dockerComposePath}`, 'down'])
        .then(() => {
            console.log(
                `${CONSOLE_PREFIX} Successfully shut down Docker container`
                    .green
            );
        })
        .catch(error => {
...
const dockerConfig = ({ flags, revision, downloadHost }) => {
