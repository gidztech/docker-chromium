...
const dockerBuild = async () => {
    let hasTriedToBuildFromScratch = false;
    try {
        ...
        if (error.message.includes('failed to build')) {
            ...
            if (!hasTriedToBuildFromScratch) {
                hasTriedToBuildFromScratch = true;
                serviceToBuild = 'chromium_build';
...
const dockerConfig = async ({ flags, revision }) => {
    ...
    if (revision) {
        ...
        // patch Dockerfile_build (alternative)
