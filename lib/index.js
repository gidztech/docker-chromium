...
const dockerUp = async () => {
    try {
        ...
        await runCommand(
            `CHROMIUM_ADDITIONAL_ARGS='${chromiumAdditionalArgs}' docker-compose`,
...
const dockerConfig = ({
    ...
    if (useDockerBuild) {
        ...
        console.log(
            `${CONSOLE_PREFIX} Use Docker build ${useDockerBuild.contextPath}, with context path ${useDockerBuild.contextPath}`
                .green
        );
    }
